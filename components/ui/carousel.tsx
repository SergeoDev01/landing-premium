...
const DEFAULT_ITEMS: CarouselItem[] = [
  {
    ...
    id: 1,
    icon: <FileText className="h-[24px] w-[24px] text-white" />
  },
  {
    title: 'Animations',
    description: 'Smooth animations for your projects.',
    id: 2,
    icon: <Circle className="h-[24px] w-[24px] text-white" />
  },
  {
    title: 'Components',
    description: 'Reusable components for your projects.',
    id: 3,
    icon: <Layers className="h-[24px] w-[24px] text-white" />
  },
  {
    title: 'Backgrounds',
    description: 'Beautiful backgrounds and patterns for your projects.',
    id: 4,
    icon: <Layout className="h-[24px] w-[24px] text-white" />
  },
  {
    title: 'Common UI',
    description: 'Common UI components are coming soon!',
    id: 5,
    icon: <Code className="h-[24px] w-[24px] text-white" />
  }
];
...
const GAP = 24;
// Animation adoucie : moins élastique, plus fluide
const SPRING_OPTIONS = { type: 'spring' as const, stiffness: 50, damping: 20, mass: 1 };
    ...
    <motion.div
      ...
      className={`relative shrink-0 flex flex-col ${
        round
          ...
          : 'items-start justify-between bg-card/40 backdrop-blur-xl border border-white/10 rounded-[20px] transition-colors outline-none'
  ...
  return (
    ...
    >
      ...
      <div className={`flex w-full justify-center ${round ? 'absolute z-20 bottom-12 left-1/2 -translate-x-1/2' : ''}`}>
        <div className="mt-8 flex w-[200px] justify-between px-8">
          {items.map((_, index) => (
            <motion.div
              key={index}
